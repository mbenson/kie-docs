...
So far the data and the matching process have been simple and small.
...
Rule Matches on the Agenda are referred to as a [term]_conflict set_
 and their execution is determined by a conflict resolution strategy.
...
The stack has push/pop behaviour.
...
First we set the focus to the "report" group and then by placing the focus on "calculation" we ensure that group is evaluated first:
